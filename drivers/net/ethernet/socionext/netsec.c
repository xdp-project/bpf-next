				  ...
				  dma_addr_t *dma_handle, u16 *desc_len)
{
	...
	*dma_handle = page_private(page) + NETSEC_RXBUF_HEADROOM;
	*desc_len = PAGE_SIZE;
			     ...
			     const struct netsec_desc *desc, void *buf)
{
	...
	dring->desc[idx] = *desc;
	if (desc->buf_type == NETSEC_SKB)
		dring->desc[idx].skb = buf;
	else if (desc->buf_type == NETSEC_XDP)
